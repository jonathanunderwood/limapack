...
asymrot_molecule_energy(const asymrot_molecule_t *mol, const int J, const int n)
{
	      ...
	      for (Jp = Jpmin; Jp <= Jpmax; Jp++)
		{
		  ...
		  for (p = -2; p <= 2; p++)
			...
			continue;
		      ...
		      for (np = -Jp; np <= Jp; np++)
			{
			  double Ep = asymrot_eigsys_eigval_get (eigsys, Jp, np);
			  ...
			  for (K = -J; K <= J; K++)
			    {
			      double a = asymrot_eigsys_eigvec_coef_get (eigsys, J, n, K);
			      gsl_complex c4;
			      int q;
			      ...
			      for (q = -2; q <= 2; q += 2)
				{
				  ...
				  ap = asymrot_eigsys_eigvec_coef_get (eigsys, Jp, np, Kp);
...
{
  //  const asymrot_molecule_t *mol = (asymrot_molecule_expval_t *) molecule;
...
asymrot_molecule_expval_mpi_recv(const molecule_t *molecule, 
				...
				int dest, int tag, MPI_Comm comm)
{
  //  asymrot_molecule_t *mol = (asymrot_molecule_expval_t *) molecule;
			     ...
			     molecule_tdse_worker_t *worker)
{
  ...
  for (J = 0; J <= mol->Jmax; J++)
	...
	{
	  int status = 
	    JKMarray_int_get(mol->job_status, J, n, M);
	  if (status == __TODO)
	    {
...
asymrot_molecule_set_tdse_job_done (molecule_t *molecule, 
				    molecule_tdse_worker_t *worker)
{
  asymrot_molecule_t *m = 
    (asymrot_molecule_t *) molecule;
  asymrot_molecule_tdse_worker_t *w = 
    (asymrot_molecule_tdse_worker_t *) worker;
		      ...
		      for (Jp = Jpmin; Jp <= Jpmax; Jp++)
			{
			  ...
			  for (Mp = Mpmin; Mp <= Mpmax; Mp++)
			      ...
			      for (np = npmin; np <= npmax; np++)
				{
				  ...
				  E_Jpnp = asymrot_eigsys_eigval_get (eigsys, Jp, np);

				  eterm =
				    gsl_complex_polar (1.0, (E_Jn - E_Jpnp) * t);
				  ...
				  for (k = kmin; k <= 2; k += 2)
				    {
				      ...
				      gsl_complex c2 = gsl_complex_mul (c1, Ekp);

				      for (K = -J; K <= J; K++)
					{
					  double a = asymrot_eigsys_eigvec_coef_get 
					    (eigsys, J, n, K);
					  int q;
					  ...
					  for (q = -k; q <= k; q += 2)
						...
						continue;
					      ...
					      ap = asymrot_eigsys_eigvec_coef_get 
						(eigsys, Jp, np, Kp);

					      if (fabs (ap) < mol->eigvecmin)
						continue;

					      alpha = polarizability_get (mol->alpha, k, -q);

					      b = -a * ap * alpha * f *
						dmtxel (J, K, M, Jp, Kp, Mp, k, p, q);
...
*/
{
  ...
  if (mol->job_status == NULL)
    {
      fprintf(stderr, "%s %d: failed to allocate memory for mol->tdse_status.\n",
	      __func__, __LINE__);
      asymrot_molecule_dtor((molecule_t *)mol);
      return NULL;
    }
...
asymrot_molecule_cfg_parse_ctor(const config_setting_t *cfg)
{
  ...
  if (mol == NULL)
    fprintf (stderr, "%s %d: failed to create asymrot_molecule structure after parsing config.\n", __func__, __LINE__);
