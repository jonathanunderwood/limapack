...
gsl_complex
laser_polzn_vec_get (laser_polzn_vec *e, const int p)
{
  ...
  for (p = -1; p <= 1; p++)
    {
      ...
      for (q = -1; q <= 1; q++)
	{
	  gsl_complex dmtx = jlib_Drot (2, 2 * q, 2 * p, phi, theta, chi);
...
laser_polzn_vector
laser_polzn_vector_spherical (const gsl_complex ex, const gsl_complex ey,
			      const gsl_complex ez)
/* Takes the cartesian components of a complex vector and returns a complex
   vector containing the spherical tensor components. */
{
  laser_polzn_vector eout;
  const double rt2 = 1.0 / sqrt (2.0);
  int ip;
  gsl_complex i = gsl_complex_rect (0.0, 1.0);
  gsl_complex iey = gsl_complex_mul (ey, i);
  ...
  /* e_-1 =  1/sqrt(2) * (e_x - i*e_y) */
  ip = VIDX (-1);
  eout.e[ip] = gsl_complex_sub (ex, iey);
  eout.e[ip] = gsl_complex_mul_real (eout.e[ip], rt2);
  ...
  /* e_+1 = - 1/sqrt(2) * (e_x + i*e_y) */
  ip = VIDX (1);
  eout.e[ip] = gsl_complex_add (ex, iey);
  eout.e[ip] = gsl_complex_mul_real (eout.e[ip], -rt2);
...
laser_polzn_tensor
laser_polzn_tensor_calc (const laser_polzn_vector e1,
			 const laser_polzn_vector e2)
/* Calculates the polarization tensor E = [e1* x e2]^k_q. Pass e1 to this
   function and not e1*. */
{
  int k;
  laser_polzn_tensor Eout;
  gsl_complex zero = gsl_complex_rect (0.0, 0.0);
      ...
      for (q = -k; q <= k; q++)
	{
	  ...
	  for (p1 = -1; p1 <= 1; p1++)
		...
		continue;
	      ...
	      c = b * jlib_3j (2, 2, 2 * k, 2 * p1, 2 * p2, -2 * q);

	      e1cc = gsl_complex_conjugate (e1.e[ip1]);

	      d = gsl_complex_mul (e1cc, e2.e[ip2]);
	      d = gsl_complex_mul_real (d, c);

	      Eout.E[ikq] = gsl_complex_add (Eout.E[ikq], d);
	    }
	}
    }
